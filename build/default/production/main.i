...
extern volatile uint8_t uart1RxBuffer[80];
extern volatile uint8_t uart2RxBuffer[80];

uint8_t PixelBase[3]={0,0,0};
uint8_t Game_array[16][16][3];
uint8_t Video_array[16][16][3];
...
void (*putch_uartfunction_ptr)(uint8_t)=UART1_Write;
...
__attribute__((inline)) void Matrix_refresh(void ) {
...
uint8_t x,y;
    ...
    for (y=0; y<16; y++) {
        for (x=0; x<16; x++) {
            Game_array[y][x][1]=red;
            Game_array[y][x][0]=green;
            Game_array[y][x][2]=blue;
        }
    }
    TMR0_StartTimer();
}
