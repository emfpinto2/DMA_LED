...
void Show_buffers(void) {
    printf("PixelBase[]\r\n");
    printf("  %02x %02x %02x\r\n",PixelBase[1],PixelBase[0],PixelBase[2]);

    printf("PixelArray[][]\r\n");
    for (uint16_t i=0; i< 10; i++) {
        printf("  #%03d %02x %02x %02x\r\n",i,Game_array[i][1], Game_array[i][0], Game_array[i][2]);
...
void my_DMA2_Initialize(void) {
    ...
    DMA2SSA = (volatile __uint24)&PixelBase;
    DMA2DSA = (volatile unsigned int)&Game_array;
    ...
    DMA2CON1bits.SMODE = 0b01;
    DMA2CON1bits.DMODE = 0b01;
    ...
    do{ __asm("BANKSEL PRLOCK"); __asm("MOVLW   0x55"); __asm("MOVWF   (PRLOCK & 0xFF)"); __asm("MOVLW   0xAA"); __asm("MOVWF   (PRLOCK & 0xFF)"); __asm("BCF     (PRLOCK & 0xFF),0"); }while(0);
    MAINPR = 3;
    ISRPR = 2;
    DMA1PR = 0;
    DMA2PR = 1;
    SCANPR = 4;
    do{ __asm("BANKSEL PRLOCK"); __asm("MOVLW   0x55"); __asm("MOVWF   (PRLOCK & 0xFF)"); __asm("MOVLW   0xAA"); __asm("MOVWF   (PRLOCK & 0xFF)"); __asm("BSF     (PRLOCK & 0xFF),0"); }while(0);
